...
class HourLines extends StatelessWidget with TimeLineUtils {
  ...
  Widget build(BuildContext context) {
    ...
    final positionedLines = segments.map((e) {
      final rangeSize = heightPerMinute * e.duration.inMinutes;
      final position = rangeSize + previousPosition;
      previousPosition = position;

      if (isHorizontal) {
        // Vertical lines for horizontal time axis
        return Positioned(
          left: position,
          top: 0,
          bottom: 0,
        ...
        // Horizontal lines for vertical time axis (default)
        return Positioned(
          top: position,
          left: 0,
          right: 0,
