...
class TimeLine extends StatelessWidget with TimeLineUtils {
  ...
  Widget build(BuildContext context) {
    ...
    final positionedTimes = segments.indexed.map((e) {
      ...
      final segmentSize = range.duration.inMinutes * heightPerMinute;
      final pos = index * segmentSize;
      ...
      if (isHorizontal) {
        // Horizontal: position labels along the x-axis
        return Positioned(
          left: pos - (itemSize.width / 2),
          child: Padding(
            padding: textPadding,
            child: Text(
    ...
    // Drag feedback times (only for vertical mode for now).
      ...
      builder: (context, visibleRange, child) {
        ...
        return ValueListenableBuilder(
          ...
          builder: (context, eventBeingDragged, child) {
            ...
            final startPos = start.difference(timeOfDayRange.start.toInternalDateTime(start)).inMinutes * heightPerMinute;
            final endPos = end.difference(timeOfDayRange.start.toInternalDateTime(end)).inMinutes * heightPerMinute;
