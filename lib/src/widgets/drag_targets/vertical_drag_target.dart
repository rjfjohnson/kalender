  ...
  static bool onWillAcceptWithDetails(
    ...
    return DragTargetUtilities.handleDragDetails(
      ...
      onResize: (event, direction) {
        final isHorizontal = viewController.viewConfiguration.isHorizontal;
        return isHorizontal ? direction.horizontal : direction.vertical;
      },
      onReschedule: (event) {
...
class _VerticalDragTargetState extends State<VerticalDragTarget> with SnapPoints, DragTargetUtilities {
  ...
  bool get _isHorizontal => viewController.viewConfiguration.isHorizontal;
  ...
  InternalDateTime? calculateCursorDateTime(
    ...
    final scrollOffset = _isHorizontal
        ? Offset(scrollController.offset, 0)
        : Offset(0, scrollController.offset);
    final localCursorPosition = calculateLocalCursorPosition(offset, scrollOffset: scrollOffset);
    if (localCursorPosition == null) return null;

    // When horizontal: dy selects the day (cross-axis), dx selects the time.
    // When vertical (default): dx selects the day (cross-axis), dy selects the time.
    final crossAxisPos = _isHorizontal ? localCursorPosition.dy : localCursorPosition.dx;
    final timeAxisPos = _isHorizontal ? localCursorPosition.dx : localCursorPosition.dy;

    final cursorDateIndex = (crossAxisPos / dayWidth).floor().clamp(0, visibleDates.length - 1);
    ...
    final durationFromStart = timeAxisPos ~/ heightPerMinute;
  ...
  CalendarEvent? resizeEvent(CalendarEvent event, ResizeDirection direction, InternalDateTime cursorDateTime) {
    // Accept resize along the time axis: vertical when vertical, horizontal when horizontal.
    if (_isHorizontal) {
    ...
    final dateTimeRange = switch (direction) {
      ...
      ResizeDirection.left => calculateDateTimeRangeFromStart(internalRange, cursorSnapPoint),
      ResizeDirection.right => calculateDateTimeRangeFromEnd(internalRange, cursorSnapPoint),
    };
