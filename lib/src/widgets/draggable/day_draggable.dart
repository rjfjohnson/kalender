...
class _DayDraggableState extends State<DayDraggable> with NewDraggableWidget {
  ...
  bool get _isHorizontal {
    final vc = context.calendarController.viewController;
    return vc is MultiDayViewController && vc.viewConfiguration.isHorizontal;
  }
  ...
  Widget build(BuildContext context) {
                  ...
                  child: context.interaction.allowEventCreation
                      ? switch (context.interaction.createEventGesture) {
                          CreateEventGesture.tap => Draggable(
                              ...
                              child: isHorizontal
                                  ? Container(color: Colors.transparent, width: widget.pageHeight)
                                  : Container(color: Colors.transparent, height: widget.pageHeight),
                            ),
    ...
    // When horizontal, days stack vertically. When vertical, days spread horizontally.
    return Listener(
      child: isHorizontal ? Column(children: dayWidgets) : Row(children: dayWidgets),
    );
  }
  ...
  InternalDateTime _calculateTimeAndDate(InternalDateTime date, Offset localPosition) {
    // Use the appropriate axis for the time position.
    final timeAxisPosition = _isHorizontal ? localPosition.dx : localPosition.dy;
    final durationFromStart = timeAxisPosition ~/ context.heightPerMinute;
