...
class _PositionedTimeIndicatorState extends State<PositionedTimeIndicator> {
  ...
  Widget build(BuildContext context) {
    final isHorizontal = widget.viewController.viewConfiguration.isHorizontal;

    return LayoutBuilder(
      builder: (context, constraints) {
        final isHidden = pageOffset <= -_visibilityThreshold || pageOffset >= _visibilityThreshold;
        final indicator = isHidden
        ...
        if (isHorizontal) {
          // Horizontal mode: days are stacked vertically, so position indicator with top/bottom
          final pageHeight = constraints.maxHeight;
          final dayHeight = pageHeight / widget.viewController.viewConfiguration.numberOfDays;

          // Calculate the vertical position for today
          final topPos = (pageOffset * pageHeight) + (todayIndex * dayHeight);
          final bottomPos = pageHeight - topPos - dayHeight;
          ...
          // Vertical mode (default): days spread horizontally, position with left/right
          final pageWidth = constraints.maxWidth;
          final dayWidth = pageWidth / widget.viewController.viewConfiguration.numberOfDays;

          final leftPos = left(pageWidth, dayWidth);
          final rightPos = pageWidth - leftPos - dayWidth;
